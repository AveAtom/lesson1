    ...
    def personal_sum(numbers):  # Должна принимать коллекцию numbers
        ...
        for number in numbers:
            try:  # Засовываем проверку внутрь for (иначе не работает). Если использовать конструкцию sum([number for number in numbers])
                # то придется вводить отдельную функцию проверки sum([correct_elem(number) for number in numbers])
                result += number  # Штатная работа функции
            except TypeError:
                print(
                    f'Некорректный тип данных для подсчёта суммы - {number}')  # Выводим строку ошибки + ошибочное значение
                incorrect_data += 1  # Увеличиваем счетчик ошибок (нужен для вычисления среднего значения).
        return result, incorrect_data  # Если возвращаемое значение кортеж - скобки можно не писать.
