...
def indy_work(code=0):  # Определяем возвращаемую функцию indy_work.
    ...
    for i in range(1,code): # Внешний цикл перебора опорных чисел. Для числа 10 опорные числа будут 1..9.
        for j in range(2,code): # Внутренний цикл чисел перебора. Для числа 10 числа перебора будут 2..9.
            # Условия выбора пары:
            # - code нацело делится на (i+j).
            # - поскольку для некоторых пар уже может быть найдена пара с цифрами в обратном порядке - проверяем:
            # есть ли такая пара. Если есть - найденную пару отбрасываем.
            # - в переборе не должно быть пар одинаковых чисел.
