...
# *************************************************************************************************
# === Функции ===
def get_multiplied_digits(number=0): # Определяем функцию get_multiplied_digits, которая принимает аргумент целое число
                                    # number и подсчитывает произведение цифр этого числа.
    str_number = str(number) # Создаем переменную str_number
    first=int(str_number[0]) # Создаем переменную first и запишем в неё первый символ из
                                # str_number в числовом представлении(int).

    if len(str_number)==1: # Если длина в str_number одно число - завершаем рекурсию.
        return first
    else: #  иначе, отнимаем от str_number символ слева, и преобразовав в число - отправляем в рекурсию.
